    ...
    public class IonTcpConnection
    {
        ...
        public void SendData(byte[] Data)
        {
            if (this.Alive)
            {
                ...
                {
                    AleedaEnvironment.GetLog().WriteUnhandledExceptionError("IonTcpConnection.Send", ex);
                }
            }
        }
        public void SendData(string sData)
        {
            SendData(AleedaEnvironment.GetDefaultTextEncoding().GetBytes(sData));
        }
        public void SendMessage(ServerMessage message)
        {
            ...
            Console.WriteLine(" [{0}] <-- {1}{2}", mID, message.Header, message.GetContentString());
            //Console.ResetColor();
        }

        public void Send_Data(string message)
        {
            SendData(message.ToString() + Convert.ToChar(1));

            /*Console.ForegroundColor = ConsoleColor.DarkGreen;
            Console.WriteLine(" [{0}] <-- {1}{2}", mID, message.ToString(), Convert.ToChar(1));
            Console.ResetColor();*/
        }
        ...
        private void WaitForData()
        {
            if (this.Alive)
            {
                ...
                {
                    AleedaEnvironment.GetLog().WriteUnhandledExceptionError("IonTcpConnection.WaitForData", ex);
                    ConnectionDead();
                }
            }
        }
