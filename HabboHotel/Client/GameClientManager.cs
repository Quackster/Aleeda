...
namespace Aleeda.HabboHotel.Client
{
    ...
    public class GameClientManager
    {
        ...
        {
            int interval = AleedaEnvironment.Configuration.TryParseInt32("projects.habbo.server.pinginterval");
            while (true)
            {
                try
                {
        ...
        public void StopClient(uint clientID)
        {
            ...
            if (client != null)
            {
                ...
                AleedaEnvironment.GetTcpConnections().DropConnection(clientID);
                ...
                // Log event
                //AleedaEnvironment.GetLog().WriteInformation("Stopped client " + clientID);
            }
        }
        ...
        public GameClient GetClientOfHabbo(string accountName)
        {
            lock (mClients)
            {
                foreach (GameClient client in mClients.Values)
                {
                    if (client.GetHabbo() != null && client.GetHabbo().Username == accountName)
                    {
                        return client;
                    }
                }
