...
namespace Aleeda.HabboHotel.Habbos
{
    public class HabboAuthenticator
    {
        ...
        public Habbo Login(string sUsername, string sPassword)
        {
            ...
            if (habbo.LoadByUsername(AleedaEnvironment.GetDatabase(), sUsername) == false)
            ...
            AleedaEnvironment.GetHabboHotel().GetClients().KillClientOfHabbo(habbo.ID);
        ...
        public Habbo Login(string sTicket)
        {
            // Do not use HabboManager.GetHabbo(string) here, as caching is planned to be implemented there
            Habbo habbo = new Habbo();
            if (!habbo.LoadBySsoTicket(AleedaEnvironment.GetDatabase(), sTicket))
            {
